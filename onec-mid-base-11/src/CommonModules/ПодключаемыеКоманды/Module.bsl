...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыРазмещения = Неопределено) Экспорт
	...
	// {{Студент 2024-03-18  
    ОЛ_ИнтерфейсФормыСервер.ДобавитьНаФормуДополнителныеЭлементы(Форма);
    
    Если ИмяФормы = "Документ.ЗаказПокупателя.Форма.ФормаДокумента" Тогда       
        Форма.ИменаТЧ = СписокТЧ(Форма);       
    КонецЕсли;
    // Студент}}
...
Функция СписокТЧ(Форма)
 	
 	// {{Студент 2024-05-31  
	 	СписокТЧ = Новый СписокЗначений;
	    
	    ТабЧастиМетаданные = Форма.Объект.Ссылка.Метаданные().ТабличныеЧасти;
	  
	    Для Каждого ТабЧасть Из ТабЧастиМетаданные Цикл
	    
	        СписокТЧ.Добавить(ТабЧасть.Имя);	
